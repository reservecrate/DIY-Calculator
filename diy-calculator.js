...
digits.forEach(digit => { // Add the pressed digits to operandsStr
    digit.addEventListener('click', () => {
        digit.id = 'clicked';
        operandsSubArr.push(digit.textContent);
    })
})
...
operators.forEach(operator => { // Add the pressed operators to operatorsArr, add a space to operands.textContent to serve as a delimiter when creating operandsArr
    operator.addEventListener('click', () => {
        document.querySelector('#clicked').id = 'ph';
        operator.id = 'clicked';
        operatorsArr.push(operator.textContent); // push the pressed operator to operatorsArr
        operandsSubArr.push(' '); // push a space to operandsSubArr to serve as a delimiter
        operandsSubArr = [...operandsSubArr.join('').trim().split(' ').map(elem => +elem)];
        operandsArr.push(...operandsSubArr);
        operandsSubArr = [];
        ...
        if (operandsArr.length >= 2) {
            ...
            operatorsArr.shift();
            operations.textContent = resultNum + operatorsArr[0];
            operandsArr = [];
